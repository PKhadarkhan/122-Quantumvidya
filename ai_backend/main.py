...
@app.post("/exam")
async def exam(req: ExamRequest):
    if not req.course or not req.subject or not req.level:
        raise HTTPException(status_code=400, detail="Course, subject, and level are required.")
    return await generate_exam(req.course, req.subject, req.level)
...
def api_login(user: UserLogin):
    ...
    for u in users:
        if u.get("email") == user.email and u.get("password") == user.password:
            return {"message": "Login successful", "user": {"name": u.get("name"), "email": u.get("email")}}

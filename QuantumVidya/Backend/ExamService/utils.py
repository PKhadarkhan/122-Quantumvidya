...
class DummyFaceRecognition:
    ...
    def face_encodings(self, *args, **kwargs): return [[0.0]]
    def face_locations(self, *args, **kwargs): return [(100, 200, 200, 100)]
    def compare_faces(self, *args, **kwargs): return [True]
    def face_distance(self, *args, **kwargs): return [0.0]
try:
        ...
        class face_detection:
            class FaceDetection:
                ...
                def process(self, img):
                    class DummyResults: detections = []
                    return DummyResults()
        class drawing_utils: pass
mp = DummyMediapipe()
import numpy as np
import time
import math
...
class DummyStream:
    def read(self, chunk): return b'\x00' * (chunk * 2)
class DummyPyAudio:
    class PyAudio:
        def open(self, *args, **kwargs): return DummyStream()
    paInt16 = 8
try:
    import pyaudio
except ImportError:
...
f_name_directory = os.path.join(os.getcwd(), 'static', 'OuputAudios')
# Capture
cap = None
...
def reduceBitRate (input_file,output_file):
   ...
   print("Bitrate conversion bypassed, copied instead.")
...
class FaceRecognition:
    ...
    def encode_faces(self):
        for image in os.listdir('../../Frontend/ExamStatic/Profiles'):
            face_image = face_recognition.load_image_file(f"../../Frontend/ExamStatic/Profiles/{image}")
